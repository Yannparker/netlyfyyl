...
const TicketComponent: React.FC<TicketComponentProps> = ({
  ...
  const [waitTimeStatus, setWaitTimeStatus] = useState<'success' | 'error'>(
    'success'
  )
  ...
  useEffect(() => {
    ...
    setWaitTimeStatus(
      totalWaitTime !== 0 && elapsedMinutes > totalWaitTime ? 'error' : 'success'
    )
            ...
            {totalWaitTime !== 0 && (
              <li>
                ...
                <div className='timeline-middle'>
                  <StatusIcon success={waitTimeStatus === 'success'} />
                </div>
                <div
...
const StatusIcon = ({ success }: { success: boolean }) => (
  <svg
    ...
    viewBox='0 0 20 20'
    fill='currentColor'
    className={`h-5 w-5 ${success ? 'text-green-500' : 'text-red-500'}`}
  >
    <path
      fillRule='evenodd'
